...
constexpr const char* kObjFragmentShader =
    ...
    AndroidViewerApp::AndroidViewerApp(JavaVM* vm, jobject obj, jobject asset_mgr_obj)
  ...
  head_tracker_ = CardboardHeadTracker_create();
    ...
    AndroidViewerApp::~AndroidViewerApp() {
  CardboardHeadTracker_destroy(head_tracker_);
  CardboardLensDistortion_destroy(lens_distortion_);
  CardboardDistortionRenderer_destroy(distortion_renderer_);
}

void AndroidViewerApp::OnSurfaceCreated(JNIEnv* env) {
  ...
  ANDROIDVIEWER_CHECK(room_.Initialize(env, asset_mgr_, "CubeRoom.obj",
                                 obj_position_param_, obj_uv_param_))
  ANDROIDVIEWER_CHECK(
      room_tex_.Initialize(env, java_asset_mgr_, "CubeRoom_BakedDiffuse.png"))
  ANDROIDVIEWER_CHECK(target_object_meshes_[0].Initialize(
      env, asset_mgr_, "Icosahedron.obj", obj_position_param_, obj_uv_param_))
  ANDROIDVIEWER_CHECK(target_object_not_selected_textures_[0].Initialize(
      env, java_asset_mgr_, "Icosahedron_Blue_BakedDiffuse.png"))
  ANDROIDVIEWER_CHECK(target_object_selected_textures_[0].Initialize(
      env, java_asset_mgr_, "Icosahedron_Pink_BakedDiffuse.png"))
  ANDROIDVIEWER_CHECK(target_object_meshes_[1].Initialize(
      env, asset_mgr_, "QuadSphere.obj", obj_position_param_, obj_uv_param_))
  ANDROIDVIEWER_CHECK(target_object_not_selected_textures_[1].Initialize(
      env, java_asset_mgr_, "QuadSphere_Blue_BakedDiffuse.png"))
  ANDROIDVIEWER_CHECK(target_object_selected_textures_[1].Initialize(
      env, java_asset_mgr_, "QuadSphere_Pink_BakedDiffuse.png"))
  ANDROIDVIEWER_CHECK(target_object_meshes_[2].Initialize(
      env, asset_mgr_, "TriSphere.obj", obj_position_param_, obj_uv_param_))
  ANDROIDVIEWER_CHECK(target_object_not_selected_textures_[2].Initialize(
      env, java_asset_mgr_, "TriSphere_Blue_BakedDiffuse.png"))
  ANDROIDVIEWER_CHECK(target_object_selected_textures_[2].Initialize(
...
void AndroidViewerApp::SetScreenParams(int width, int height) {
  screen_width_ = width;
  screen_height_ = height;
  screen_params_changed_ = true;
}

void AndroidViewerApp::OnDrawFrame() {
...
void AndroidViewerApp::OnTriggerEvent() {
  if (IsPointingAtTarget()) {
    HideTarget();
  }
}

void AndroidViewerApp::OnPause() { CardboardHeadTracker_pause(head_tracker_); }

void AndroidViewerApp::OnResume() {
...
void AndroidViewerApp::SwitchViewer() {
  CardboardQrCode_scanQrCodeAndSaveDeviceParams();
}

bool AndroidViewerApp::UpdateDeviceParams() {
...
void AndroidViewerApp::GlTeardown() {
  if (framebuffer_ == 0) {
    return;
  }
...
Matrix4x4 AndroidViewerApp::GetPose() {
...
void AndroidViewerApp::DrawWorld() {
  DrawRoom();
  DrawTarget();
}

void AndroidViewerApp::DrawTarget() {
  glUseProgram(obj_program_);
...
void AndroidViewerApp::DrawRoom() {
  glUseProgram(obj_program_);
...
void AndroidViewerApp::HideTarget() {
  cur_target_object_ = RandomUniformInt(kTargetMeshCount);
...
bool AndroidViewerApp::IsPointingAtTarget() {
