...
import com.zelgius.remoteController.hexapod.HexapodController
...
val datagramManager = DatagramManager(false) {}
...
fun main(args: Array<String>) = runBlocking {
    ...
    try {
        ...
        val hexapod = HexapodController(driver1, driver2).apply {
            reset()
            run()
        }

        val commandManager = CommandManager(hexapod, arm, led, analogDriver)
        commandManager.updateStatus(true)
        ...
        clientManager.controlListener = {
            runBlocking {
                commandManager.handleControl(it.toRemoteControl())
            }
        }

        while (true) {
            delay(500)
            commandManager.updateStatus(false)
            commandManager.status?.let {
                clientManager.sendStatus(it)
            }
        }
    } catch (e: Exception) {
        // Initialization fail
