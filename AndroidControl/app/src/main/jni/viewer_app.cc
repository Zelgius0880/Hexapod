...
// Simple shaders to render .obj files without any lighting.
    ...
    ViewerApp::ViewerApp(JavaVM *vm, jobject obj, jobject asset_mgr_obj)
    ...
    void ViewerApp::OnSurfaceCreated(JNIEnv *env) {
        ...
        VIEWER_CHECK(room_.Initialize(env, asset_mgr_, "CubeRoom.obj",
                                      obj_position_param_, obj_uv_param_));
        VIEWER_CHECK(
                room_tex_.Initialize(env, java_asset_mgr_, "CubeRoom_BakedDiffuse.png"));
        VIEWER_CHECK(target_object_meshes_[0].Initialize(
                env, asset_mgr_, "Icosahedron.obj", obj_position_param_, obj_uv_param_));
        VIEWER_CHECK(target_object_not_selected_textures_[0].Initialize(
                env, java_asset_mgr_, "Icosahedron_Blue_BakedDiffuse.png"));
        VIEWER_CHECK(target_object_selected_textures_[0].Initialize(
                env, java_asset_mgr_, "Icosahedron_Pink_BakedDiffuse.png"));
        VIEWER_CHECK(target_object_meshes_[1].Initialize(
                env, asset_mgr_, "QuadSphere.obj", obj_position_param_, obj_uv_param_));
        VIEWER_CHECK(target_object_not_selected_textures_[1].Initialize(
                env, java_asset_mgr_, "QuadSphere_Blue_BakedDiffuse.png"));
        VIEWER_CHECK(target_object_selected_textures_[1].Initialize(
                env, java_asset_mgr_, "QuadSphere_Pink_BakedDiffuse.png"));
        VIEWER_CHECK(target_object_meshes_[2].Initialize(
                env, asset_mgr_, "TriSphere.obj", obj_position_param_, obj_uv_param_));
        VIEWER_CHECK(target_object_not_selected_textures_[2].Initialize(
                env, java_asset_mgr_, "TriSphere_Blue_BakedDiffuse.png"));
        VIEWER_CHECK(target_object_selected_textures_[2].Initialize(
    ...
    void ViewerApp::SetScreenParams(int width, int height) {
        screen_width_ = width;
        screen_height_ = height;
        screen_params_changed_ = true;
    }

    void ViewerApp::OnDrawFrame(jbyte* left, jbyte* right) {
        ...
        for (int eye = 0; eye < 2; ++eye) {
            ...
            //TODO draw bytes here
            glBindTexture(GL_TEXTURE_2D, textures_[0]);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, screen_width_, screen_height_, 0, GL_RGBA, GL_UNSIGNED_BYTE, left);

            glBindTexture(GL_TEXTURE_2D, textures_[1]);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, screen_width_, screen_height_, 0, GL_RGBA, GL_UNSIGNED_BYTE, right);
            //DrawWorld();
        }
    ...
    void ViewerApp::OnTriggerEvent() {
        if (IsPointingAtTarget()) {
            HideTarget();
        }
    }

    void ViewerApp::OnPause() { CardboardHeadTracker_pause(head_tracker_); }
    ...
    void ViewerApp::SwitchViewer() {
        CardboardQrCode_scanQrCodeAndSaveDeviceParams();
    }

    bool ViewerApp::UpdateDeviceParams() {
    ...
    void ViewerApp::GlSetup() {
        ...
        left_eye_texture_description_.texture = textures_[0];
        left_eye_texture_description_.left_u = 0;
        left_eye_texture_description_.right_u = 0.5;
        left_eye_texture_description_.top_v = 1;
        left_eye_texture_description_.bottom_v = 0;

        right_eye_texture_description_.texture = textures_[1];
    ...
    bool ViewerApp::IsPointingAtTarget() {
